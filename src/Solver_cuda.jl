...
"""
function solve_one_shot_cuda(W, M, H, a_gpu, G, dt, nt, M_order, vc=nothing;
    ...
    solve_elastic_cuda!(W, M, H, a_gpu, G_isrc, dt, nt, M_order, vc)
    ...
    if output_shot_png
        save_shot_gather_raw(rec_data_cpu, dt, "shot$i_src-$rec_type.png";
            title="Shot Gather $rec_type (GPU)")
    end

    if output_shot_bin
        save_shot_gather_bin(rec_data_cpu, "shot$i_src-$rec_type.bin")
    end
end
...
"""
function run_multi_shots_cuda(W, M, H, a_gpu, G, dt, nt, M_order;
    vc::Union{VideoConfig,Nothing}=nothing,
    output_shot_png::Bool=false,
    output_shot_bin::Bool=true)

    n_sources = length(G.sources.i0)
    @info "Starting GPU Multi-shot sequence" TotalShots = n_sources
    ...
    for i_src in 1:n_sources
        ...
        # Handle video naming per shot
        if vc !== nothing
            vc.filename = "shot$i_src-$(vc.mode).mp4"
        end

        solve_one_shot_cuda(W, M, H, a_gpu, G, dt, nt, M_order, vc;
            i_src=i_src,
